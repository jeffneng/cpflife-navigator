...
st.warning(
    f"**Scope:** this simulator only covers members turning age 55 between **{MIN_YEAR} and "
    f"{MAX_YEAR}**. See Methodology for why.",
    icon="⚠️",
)
...
with col1:
    ...
    st.markdown(
        """
        Enter your current age (and which retirement sum — BRS, FRS, or ERS — you're aiming
        for at 55), plus gender and payout start age, to see:
...
with col2:
    ...
    st.markdown(
        ...
        how Basic Plan's drawdown works, retirement sum projections, or anything about
        your own scenario from the simulator — and get an answer grounded in the same
        sourced data used by the simulator, not a generic web answer.
        """
    )
...
with st.expander("⚠️ Estimate only — please read"):
    st.markdown(
        ...
        specific cohort, and can only be confirmed via your **myCPF** account.

        **This simulator only supports members turning age 55 between {MIN_YEAR} and
        {MAX_YEAR}.** CPF has only published retirement sum (BRS/FRS/ERS) and payout
        figures for the {rsp['baseCohortYear']} cohort — for other years within that
        range, this app projects the retirement sums forward at an assumed
        {rsp['annualGrowthRate']*100:.1f}%/yr (its own assumption, not a CPF figure).
        See the **Methodology** page for exactly which figures are official and which
        are modeled approximations.
        """
    )
